...
import com.magbel.model.AccountingCreditDetails;
import com.magbel.model.AccountingDebitDetails;
...
@Path("/Service")
public class Service extends ChartCreator {
	...
	public String Chart(@QueryParam("branchCode") String branchCode,
			@QueryParam("branchRestriction") String branchRestriction) throws Exception {
	//log.setup("Starting to call pie chart!!!");
		String sql ="";
	...
	Connection con = null;
	try {
		if(branchRestriction.equals("Y")) {
		     sql = "select SUM(cost_price) from AM_ASSET a, am_ad_category c\r\n" + 
		     		"where a.category_code = c.category_code \r\n" + 
		     		"and substring(c.category_acronym,1,1) not in ('U','F') and a.asset_status='ACTIVE' and a.branch_Code =?";
		     con = getConnection("legendPlusService");
		     ps = con.prepareStatement(sql);
		     ps.setString(1, branchCode);
			} else if(branchRestriction.equals("N")) {
			 sql = "select SUM(cost_price) from AM_ASSET a, am_ad_category c\r\n" + 
			 		"where a.category_code = c.category_code \r\n" + 
			 		"and substring(c.category_acronym,1,1) not in ('U','F') and a.asset_status='ACTIVE'";
		...
		rs = ps.executeQuery();
		while(rs.next()) {
			String totalSum = rs.getString(1);
	...
	@Path("/doBarChart")
	@Produces(MediaType.APPLICATION_JSON) 
	public String DonutChart(@QueryParam("branchCode") String branchCode) throws Exception {
		//log.setup("Starting to call bar chart!!!");
		String categoryCode=null;
		String totalPrice=null;
		List<Chart> chart = getChart(branchCode);
		Chart chartField = new Chart();
		JSONArray allDataArray = new JSONArray();
		try {
			for(int i=0; i<chart.size(); i++) {
				JSONObject data = new JSONObject();
				chartField.setCategory_Code(chart.get(i).getCategory_Code());
				chartField.setTotalCostPrice(chart.get(i).getTotalCostPrice());
				
				categoryCode = chartField.getCategory_Code();
				totalPrice = chartField.getTotalCostPrice();
					
				data.put("categoryCode", categoryCode);
		   		data.put("totalPrice", totalPrice);
				
				 allDataArray.put(data);
			}	
			
			//log.setup("The feed is : " + feeds);
			//log.setup("Displayed Successfully!!!");
		}catch(Exception e) {
			e.getMessage();
		}
	...
	@Produces(MediaType.APPLICATION_JSON) 
	public String assetCreation(NewAssetRequest request) throws Throwable {
											...
											//	 double TotalCostPrice = Double.parseDouble(comp.findObject("select sum(cost_price) from NEW_GROUP_ASSET_INTERFACE where INTEGRIFY_ID = '"+integrifyId+"' AND ASSET_TYPE = 'U'"));
											 ...
											 if(assettype.equalsIgnoreCase("C")&& errorMessage.equalsIgnoreCase("") && (noofitems!=0)){
												...
												// System.out.println(" strCostPrice: "+strCostPrice );
												 String costPrice = String.valueOf(request.Cost_Price);
...
//										 } //Integrify Exist
	...
	public String getPayload() throws Exception {
		System.out.println("Starting to getPayload!!!");
		JSONObject json = new JSONObject();
			JSONArray jArray =  new JSONArray();
			JSONObject json2 = new JSONObject();
			JSONObject json3 = new JSONObject();
			
			json3.put("totalCount", 329);
			json3.put("processed", 327);
			json3.put("pending", 0);
			json3.put("failed", 2);
			json3.put("suspense", 0);
			
			json2.put("batch",json3);
			json2.put("status", "Partial");
			json2.put("statusDesc", "Posting is incomplete and there is at least one failed entry");
			jArray.put(json2);
			json.put("successful", true);
			json.put("message", "Record Successfully Retrieved");
	...
	@GET 
	@Path("/getBatchPosting")
	@Produces(MediaType.APPLICATION_JSON) 
	public String getBatchPosting() throws Exception {
		System.out.println("Starting to get Batcch Posting!!!");
		JSONObject json = new JSONObject();
			JSONArray jArray =  new JSONArray();
			JSONObject json2 = new JSONObject();
			JSONObject json3 = new JSONObject();
			...
			json2.put("batchNo", "19ah");
			json2.put("currNo", "0");
			json2.put("seqNo", "1");
			json2.put("errorCode", "DE-BAT10");
			json2.put("parameters", ",");
			json2.put("errorMessage", "The batch is not balanced.");
			
			
			json3.put("batchNo", "19ah");
			json3.put("currNo", "27");
			json3.put("seqNo", "1");
			json3.put("errorCode", "DE-VAL-002");
			json3.put("parameters", "1001050278");
			json3.put("errorMessage", "Account is Closed.");
			
			json4.put("batchNo", "19ah");
			json4.put("currNo", "27");
			json4.put("seqNo", "2");
			json4.put("errorCode", "DE-JRN74");
			json4.put("parameters", "1001050278");
			json4.put("errorMessage", "Invalid Account Number");
			
			json5.put("batchNo", "19ah");
			json5.put("currNo", "27");
			json5.put("seqNo", "3");
			json5.put("errorCode", "AC-00218");
			json5.put("parameters", "213193057~IBRAMUSE");
			json5.put("errorMessage", "The General Ledger $1 is not allowed for the Role $2");
	...
	@GET
	@Path("/getPostingDateValidation")
	@Produces(MediaType.APPLICATION_JSON) 
	public String realTimeTransactionValidation(@QueryParam("branchCode")String branchCode) throws Exception {
		System.out.println("Starting to getRealTimeValidation!!!");
		JSONObject json = new JSONObject();
		JSONArray jArray =  new JSONArray();
		...
		json3.put("postingDate", "22/06/2024");		

		json.put("data", json3);
		json.put("success", true);
		json.put("message", "Posting Date Successfully Retrieved");
		json.put("statusCode", 200);
		json.put("responseCode", "00");
		json.put("responseDescription", "SUCCESS");
	...
	@POST
	@Path("/getRealTimeValidation")
	@Produces(MediaType.APPLICATION_JSON) 
	public String postingDateValidation(String requestBody) throws Exception {
		System.out.println("Starting to getPostingDateValidation!!!");
		JSONObject json = new JSONObject();
		JSONArray jArray =  new JSONArray();
		JSONArray jArray2 =  new JSONArray();
		JSONObject json2 = new JSONObject();
		...
		json2.put("code", "ST-SAVE-052");
		json2.put("message" , "Successfully Saved and Authorized");	
		jArray.put(json2);
		
		json4.put("code", "AC-CL001");
		json4.put("message", "This transaction will cause Weekly Cash Withdrawal Limit of NGN500000 set for this customer to be exceeded");
		jArray2.put(json4);
		
		json3.put("accountId", "null");
		json3.put("coreBankingRef", "LGDGR000235");
		json3.put("trnRefNo", "001ZEXA233060J03");
		json3.put("status", "SUCCESS");
		json3.put("msgId", "6124061036067847");
		json3.put("uniqueRefNo", "null");
		json3.put("Warnings", jArray);
		json3.put("errors", jArray2);
